...
public class DeliveryTaskHandlerConfig
{
	...
	@Bean
	@Conditional(DefaultTaskHandlerType.class)
	public DAODeliveryTaskExecutor taskExecutor(@Autowired DeliveryTaskDAO deliveryTaskDAO, @Autowired DeliveryTaskHandler deliveryTaskHandler)
	{
	...
	public DefaultMessageListenerContainer jmsTaskProcessor(
		...
		@Autowired @Qualifier("jmsTransactionManager") PlatformTransactionManager jmsTransactionManager,
		@Autowired DeliveryTaskHandler deliveryTaskHandler)
	{
  ...
  public DeliveryTaskHandler deliveryTaskHandler(
		...
		@Autowired EbMSHttpClientFactory ebMSClientFactory,
		@Autowired EbMSMessageEncrypter messageEncrypter,
		@Autowired EbMSMessageProcessor messageProcessor)
	{
	...
	public static class DefaultTaskHandlerType implements Condition
	{
		...
		public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata)
		{
			return context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_START,Boolean.class,true)
					&& context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_TYPE,DeliveryTaskHandlerType.class,DeliveryTaskHandlerType.DEFAULT) == DeliveryTaskHandlerType.DEFAULT;
		}
	}
	public static class JmsTaskHandlerType implements Condition
	{
		...
		{
			return context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_START,Boolean.class,true)
					&& (context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_TYPE,DeliveryTaskHandlerType.class,DeliveryTaskHandlerType.DEFAULT) == DeliveryTaskHandlerType.JMS
					|| context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_TYPE,DeliveryTaskHandlerType.class,DeliveryTaskHandlerType.DEFAULT) == DeliveryTaskHandlerType.QUARTZ_JMS);
		}
	}
	public static class QuartzTaskHandlerType implements Condition
	{
		...
		{
			return context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_START,Boolean.class,true)
					&& (context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_TYPE,DeliveryTaskHandlerType.class,DeliveryTaskHandlerType.DEFAULT) == DeliveryTaskHandlerType.QUARTZ
					|| context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_TYPE,DeliveryTaskHandlerType.class,DeliveryTaskHandlerType.DEFAULT) == DeliveryTaskHandlerType.QUARTZ_JMS
					|| context.getEnvironment().getProperty(DELIVERY_TASK_HANDLER_TYPE,DeliveryTaskHandlerType.class,DeliveryTaskHandlerType.DEFAULT) == DeliveryTaskHandlerType.QUARTZ_KAFKA);
		}
	}
